...
class AppBloc extends Cubit<AppStates> {
    ...
    await db.transaction(
      (txn) async {
        txn
          ...
          (error) {
            debugPrint(
              'Error when insert new  ${error.toString()}',
            );
          },
        );
      },
  ...
  // ignore: non_constant_identifier_names
  void MoveDataBaseScreen({
    required String status,
    required int id,
  }) async {
    emit(
      AppMoveDataBaseScreenState(),
    );
    db.rawUpdate(
  ...
  void setIsFav(Map<dynamic, dynamic> model) {
    ...
    db
      ...
      if (kDebugMode) {
        print(
          '${model['title']} is updated successfully',
        );
      }
      emit(
        AppSetFavSuccessfulState(),
      );
      GetDataFromDataBase();
    }).catchError(
      (error) {
        if (kDebugMode) {
          print(
            'Error when update ${model['title']} ${error.toString()}',
          );
        }
        emit(
          AppSetFavErrorState(
